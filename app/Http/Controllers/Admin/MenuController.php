...
class MenuController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $chkname = Menu::where('name',$request->name)->first();
        if($chkname){
    ...
    public function edit($id)
    {
        ...
        $info = Menu::where($where)->get()->first();
    ...
    public function update(Request $request)
    {
        ...
        $duplicatename = Menu::where('name',$request->name)->where('id','!=', $request->codeid)->first();
        if($duplicatename){
            $message ="<div class='alert alert-warning'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a><b>This name already added.</b></div>";
            return response()->json(['status'=> 303,'message'=>$message]);
            exit();
